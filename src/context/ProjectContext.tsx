...
const STORAGE_KEY_PROJECTS = 'shadow_portfolio_projects_v5';
const STORAGE_KEY_PICTOGRAMS = 'shadow_portfolio_pictograms_v5';
const STORAGE_KEY_CATALOGS = 'shadow_portfolio_catalogs_v5';
const STORAGE_KEY_SETTINGS = 'shadow_portfolio_settings_v5';
const STORAGE_KEY_BRANDS = 'shadow_portfolio_brands_v5';
...
export const ProjectProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createProject = (data: Partial<Project>): Project => {
    ...
    const newProject = recomputeProject({
      ...
      cover: data.cover || '/uploads/Daewoo General Landing/Daewoo General Landing - Desktop.png',
  ...
  const createPictogramProject = (data: Partial<BrandPictogramProject>): BrandPictogramProject => {
    ...
    const newPictogram: BrandPictogramProject = {
      ...
      cover: data.cover || '/uploads/Daewoo RF Landing/Daewoo RF Landing - Desktop.png',
    ...
    try {
      ...
      if (!Array.isArray(parsed)) {
        ...
        if (parsed && typeof parsed === 'object' && parsed.projects && Array.isArray(parsed.projects)) {
          return importFullBackupJson(jsonData) as any;
        }
  ...
  const importFullBackupJson = (jsonData: string): { success: boolean; error?: string } => {
    try {
      ...
      if (parsed.projects && Array.isArray(parsed.projects)) {
        setProjects(parsed.projects.map((p: any) => recomputeProject(p)));
      }
