      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 pb-6 border-b border-white/10">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 text-xs sm:text-sm font-bold text-[#0066FF]">
            ...
            <span>کاتالوگ جامع پروژه‌ها و پلتفرم‌ها</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight leading-relaxed">
            مخزن کامل دیزاین و اسناد ({toPersianDigits(filteredProjects.length)} پروژه)
          </h1>
          <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">
            آرشیو یکپارچه تمامی وب‌سایت‌های مرجع، داشبوردها، اپلیکیشن‌ها و دیزاین‌سیستم‌های پیاده‌سازی‌شده
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-7 items-start">
        ...
        <div className="lg:col-span-8 xl:col-span-9 order-1 lg:order-2 space-y-5">
          ...
          {filteredProjects.length === 0 ? (
            ...
            /* CLEANEST SIMPLIFIED GRID VIEW */
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredProjects.map((project) => (
                <div
                  key={project.id}
                  onClick={() => onSelectProject(project)}
                  className="bg-[#0a0e1c] border border-white/10 hover:border-[#0066FF]/60 rounded-2xl p-4 transition-all duration-300 cursor-pointer group flex flex-col justify-between space-y-3.5 shadow-lg hover:shadow-2xl hover:shadow-[#0066FF]/15"
                >
                  ...
                  <div className="rounded-xl overflow-hidden bg-[#04060d] aspect-[16/10] relative border border-white/5">
                    ...
                    {/* Floating Brand Chip */}
                    <div className="absolute top-2.5 right-2.5 px-2.5 py-1 rounded-lg bg-black/75 backdrop-blur-md border border-white/15 text-white text-[11px] font-bold shadow-md">
                  ...
                  <div className="space-y-1.5 px-1">
                    <div className="flex items-center justify-between text-xs text-slate-400">
                      <span className="text-[#388bfd] font-bold">{project.typeFa}</span>
                      <span className="font-mono text-slate-400">{toPersianDigits(project.year || '—')}</span>
                    </div>
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <ArrowUpLeft className="w-4 h-4 text-slate-500 group-hover:text-[#0066FF] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all shrink-0 mt-0.5" />
                    </div>
                  </div>
