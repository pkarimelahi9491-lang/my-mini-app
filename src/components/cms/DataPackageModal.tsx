...
export const DataPackageModal: React.FC<DataPackageModalProps> = ({
  ...
  const { projects, exportJson, importJson, resetToInitial } = useProjects();
  ...
  const processFileContent = (content: string) => {
    ...
    if (res.success) {
      setImportStatus({
        success: true,
        message: `بسته ارائه با موفقیت بارگذاری شد! (${toPersianDigits(res.count)} پروژه آماده ارائه و نمایش است).`
      });
    } else {
      setImportStatus({
        success: false,
        ...
        <div className="p-5 sm:p-6 space-y-6 overflow-y-auto max-h-[75vh]">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400">
                  شامل تمام {toPersianDigits(projects.length)} پروژه ثبت‌شده، دسته‌بندی‌ها، متون و تصاویر
                </p>
              </div>
              <button
          ...
          <div className="p-5 rounded-2xl bg-white/[0.03] border border-white/10 space-y-3">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">
                فایل JSON خروجی گرفته شده را آپلود کنید تا فوراً در پلتفرم لود شود.
              </p>
            </div>
