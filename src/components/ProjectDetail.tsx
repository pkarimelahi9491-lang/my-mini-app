          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {otherProjects.map((oProj) => (
              <div
                ...
                className="bg-[#0a0e1c] rounded-2xl border border-white/10 hover:border-[#0066FF] p-4 space-y-3.5 cursor-pointer group transition-all duration-300 shadow-md hover:shadow-xl hover:shadow-[#0066FF]/10 flex flex-col justify-between"
              >
                ...
                <div className="aspect-[16/10] rounded-xl overflow-hidden bg-black/50 border border-white/5 relative">
                  <img 
                    ...
                    loading="lazy"
                  />
                  <div className="absolute top-2.5 right-2.5 px-2.5 py-1 rounded-lg bg-black/75 backdrop-blur-md border border-white/15 text-white text-[11px] font-bold">
                    {oProj.clientFa || oProj.brand}
                  </div>
                </div>

                <div className="space-y-1 pt-0.5">
                  <div className="flex items-center justify-between text-xs text-slate-400">
                    <span className="font-bold text-[#0066FF]">{oProj.typeFa}</span>
                    <span className="font-mono">{toPersianDigits(oProj.year || '—')}</span>
                  </div>
                  <h4 className="text-sm font-bold text-white group-hover:text-[#0066FF] transition-colors truncate">
                    {oProj.displayNameFa || oProj.name}
                  </h4>
                </div>

                <div className="pt-2.5 border-t border-white/5 flex items-center justify-between text-xs text-[#0066FF] font-bold">
